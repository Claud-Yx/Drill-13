...
class IdleState:
    ...
    def do(boy):
        ...
        # 벽돌과 충돌 시 (RunState, SleepState 도 동일)
        if boy.is_on_brick:
            # 벽돌의 이동량만큼 소년도 같이 이동(움직일 시에도 적용 받음)
            boy.x += boy.on_brick.speed * game_framework.frame_time
            # 벽돌의 범위 내에서만 이동 가능
            boy.x = clamp(boy.on_brick.x - 90, boy.x, boy.on_brick.x + 90 )
        else:
